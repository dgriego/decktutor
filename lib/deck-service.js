...
async function generateProfile(deck, metadata, facts, fetcher = fetch) {
  ...
  if (!response.ok) {
    ...
    const message = status === 402 ? 'AI credits are unavailable. Check the Vercel AI Gateway budget and credits.' : status === 401 || status === 403 ? `AI Gateway denied the ${credential.source} credentials (HTTP ${status}). Check the key’s team, project environment, and team model/provider permissions.` : status === 429 ? 'AI analysis is busy. Try again shortly.' : 'AI analysis is temporarily unavailable. Try again.';
