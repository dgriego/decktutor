...
const MODEL = process.env.MTGLINE_MODEL || 'openai/gpt-5.4';
...
async function gatewayToken() {
  ...
  } catch { return process.env.VERCEL_OIDC_TOKEN || null; }
}

async function generateProfile(deck, metadata, facts, fetcher = fetch) {
  const token = await gatewayToken();
  if (!token) return { profile: Core.fallbackProfile(deck, metadata, facts, 'AI analysis needs a server connection. Card-data coaching remains available.'), ai: { status: 'unavailable', code: 'configuration', message: 'AI analysis is not configured on this server.' } };
  ...
  const response = await fetcher('https://ai-gateway.vercel.sh/v1/chat/completions', { method: 'POST',
    ...
    body: JSON.stringify({ model: MODEL, messages: [{ role: 'system', content: SYSTEM }, { role: 'user', content: JSON.stringify(data) }],
      response_format: { type: 'json_schema', json_schema: { name: 'DeckProfile', strict: true, schema: PROFILE_SCHEMA } },
      max_completion_tokens: 14000, reasoning_effort: 'low' }), signal: AbortSignal.timeout(115000) });
  if (!response.ok) {
    const status = response.status;
    // Never echo provider bodies, request headers, or credentials into the client response.
    const message = status === 402 ? 'AI credits are unavailable. Check the Vercel AI Gateway budget and credits.' : status === 401 || status === 403 ? `AI Gateway denied access (HTTP ${status}). Check deployment authentication and team model/provider permissions.` : status === 429 ? 'AI analysis is busy. Try again shortly.' : 'AI analysis is temporarily unavailable. Try again.';
    return { profile: Core.fallbackProfile(deck, metadata, facts, message), ai: { status: 'unavailable', code: `provider_${status}`, message } };
  }
  const result = await response.json();
  if (result.choices?.[0]?.finish_reason !== 'stop') throw Error('The AI response was incomplete.');
  const profile = Core.validateProfile(JSON.parse(result.choices[0].message.content), deck, metadata, facts);
