...
async function generateProfile(deck, metadata, facts, fetcher = fetch) {
  ...
  if (!response.ok) {
    ...
    const message = status === 402 ? 'AI credits are unavailable. Enable credits in Vercel AI Gateway and retry.' : status === 401 || status === 403 ? 'AI Gateway is not enabled for this deployment. Enable the project’s AI connection and retry.' : status === 429 ? 'AI analysis is busy. Try again shortly.' : 'AI analysis is temporarily unavailable. Try again.';
