...
const PROFILE_TOOL = {
  type: 'function',
  function: {
    name: 'submit_deck_profile',
    description: 'Submit the validated strategy profile for the imported deck.',
...
async function generateProfile(deck, metadata, facts, fetcher = fetch) {
  ...
  const response = await fetcher('https://ai-gateway.vercel.sh/v1/chat/completions', { method: 'POST',
    ...
    body: JSON.stringify({ model: MODEL, messages: [{ role: 'system', content: SYSTEM }, { role: 'user', content: JSON.stringify(data) }],
      tools: [PROFILE_TOOL], tool_choice: { type: 'function', function: { name: 'submit_deck_profile' } },
  ...
  if (!['stop', 'tool_calls'].includes(choice?.finish_reason)) throw Error('The AI response was incomplete.');
  const call = choice.message?.tool_calls?.find(c => c.type === 'function' && c.function?.name === 'submit_deck_profile');
  if (!call?.function?.arguments) throw Error('The AI response did not return a deck profile.');
  const profile = Core.validateProfile(JSON.parse(call.function.arguments), deck, metadata, facts);
