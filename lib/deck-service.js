...
async function resolveCards(deck, fetcher = fetch) {
  ...
  for (let i = 0; i < entries.length; i += 75) {
    ...
    function collect(data, requested) {
      for (const entry of requested) {
        const card = (data.data || []).find(c => {
          const faceNames = (c.card_faces || []).map(f => f.name);
          const aliases = [c.name, ...faceNames, faceNames.join(' // ')].map(n => n.toLowerCase());
    ...
    if (missing.length) { await pause(120); collect(await collection(missing.map(c => ({ name: c.name.split(' // ')[0] })), fetcher), missing); }
  }
...
async function analyzeDeck(input, { analyze = true, fetcher = fetch } = {}) {
  ...
  if (!pending.has(key)) pending.set(key, (async () => {
    try {
      ...
      const known = /^(Invalid profile (text|list)\.|Profile references a card outside this deck\.|Invalid card role\.|Invalid card note\.|Duplicate profile cards\.|Invalid mulligan targets\.|Invalid deck tempo\.|Invalid commander priorities\.|Invalid hand weights\.|The AI response was incomplete\.)$/;
      const detail = error.name === 'TimeoutError' ? 'AI analysis exceeded the time limit.' : error instanceof SyntaxError ? 'AI returned invalid JSON.' : known.test(error.message) ? error.message : 'AI analysis could not be completed.';
      return { profile: Core.fallbackProfile(deck, metadata, facts, detail), ai: { status: 'unavailable', code: error.name === 'TimeoutError' ? 'analysis_timeout' : 'analysis_failed', message: detail + ' Retry analysis.' } };
